...
export default function Profile() {
    ...
    const enviarFoto = async (formData: any) => {
        ...
        axios.put('http://localhost:5000/api/users/avatar', formData, config).then((res) => {
            console.log(res.data);
            dispatch(updateUser(res.data.data))
        })
            .catch((err) => {
        ...
        <div className='flex bg-cover bg-top bg-no-repeat h-screen' style={{backgroundImage: `url('/assets/img/chill2.png')`}}>
            ...
            <div className='w-5/5 ml-20 mt-10'>
                <form method="PUT" onSubmit={onSubmit} className='ml-10 mt-5'>
                    <h1 className="font-bold capitalize text-xl text-center text-white mr-16 py-4">Account Overview</h1>
                    <div className='flex'>
                        <div className=''>
                        ...
                        <div className="w-full md:w-1/2 py-3 px-2 mb-6 md:mb-0">
                            ...
                            <button type='submit' className="bg-purple-500 hover:bg-purple-700 text-white font-bold py-2 px-4  rounded-full">Guardar Datos</button>
                        </div>
                ...
                <div className=''>
                    <form encType='multipart/form-data' onSubmit={onHandle} method="PUT">
                        <div className="w-full py-1 md:w-1/2 px-2 mb-6 md:mb-0">
                            <label className="block uppercase tracking-wide text-white text-xs ml-10 font-bold mb-2" htmlFor="grid-first-imagen">
                                Carga tu avatar </label>
                            <input id='image' onChange={e => setArchivo(e.target.files![0])} size={20} className="ml-10" type="file" />
                        </div>
                        <button type='submit' className="bg-purple-500 hover:bg-purple-700 text-white font-bold py-2 mt-2 ml-12 px-4 rounded-full">Guardar foto</button>
                    </form>
                </div>
            </div>
        </div>
