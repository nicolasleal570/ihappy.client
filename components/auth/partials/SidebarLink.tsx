  ...
  if (isDropdown) {
    link = (
      <div className="w-full">
        ...
        >
          <h1
            className={`hover:bg-purple-500 flex items-center p-2 ${
        ...
        {isOpen && (
          <ul>
            {options?.map((option: { url: string; name: string }, index) => (
              <li
                key={index}
                className={`'px-2 py-1 cursor-pointer text-white text-md hover:bg-purple-500 hover:text-white'`}
              >
