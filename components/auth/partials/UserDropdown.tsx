    ...
    return (
        <>
            <div className='relative ml-auto'>
                ...
                >
                    {/*  */}
                    <div className="flex bg-purple-700 w-12 h-12 rounded-full overflow-hidden flex justify-center items-center">
                        <img className='w-full h-full object-cover' src={user?.avatar} alt='profile' />
                    </div>
                    <p className="w-32 truncate px-2">{user?.first_name} {user?.last_name}</p>
                    <KeyboardArrowDownIcon className="text-lg" />
                </button>

                {/* Dropdown */}
                <div className={`w-48 ${isOpen ? 'absolute' : 'hidden'} text-left z-50 mt-2 bg-gray-200 border border-gray-300 rounded shadow-md right-0`}>
                    <Link href='/profile'>
